...
const TableHeaders = () => (
  <TableHeader>
    <TableRow>
      ...
      <TableHead className="text-right hidden md:table-cell">En Yüksek</TableHead>
      <TableHead className="text-right pr-4 hidden md:table-cell">En Düşük</TableHead>
    </TableRow>
  </TableHeader>
);
...
const CurrencyRow = ({ code, currency }: CurrencyRowProps) => (
  <TableRow key={code}>
    <TableCell className="group py-5 px-4">
      <Link to={`/doviz/${currency.tr_slug}`} className="flex items-center hover:text-blue-500">
    ...
    <TableCell className="text-right py-4">
      <Badge className={getCurrencyBadgeColor(parseFloat(currency.change))}>
